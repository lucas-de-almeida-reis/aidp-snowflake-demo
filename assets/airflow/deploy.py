#!/usr/bin/env python3
"""
deploy.py — Build, push, and deploy the Airflow orchestrator to OCI Container Instances.

This is the bridge between an external scheduler (Airflow) and AIDP. The
deployed Container Instance runs `airflow standalone` with the medallion DAG
(`full_pipeline.py`) and is granted IAM resource-principal access to AIDP's
control plane — so it can trigger jobRuns without any private key in env vars.
...
Prerequisites:
  ...
  - A Dynamic Group + Policy granting the deployed Container Instance permission
    to call the AIDP control plane. The script creates these for you with
    --create-iam, or prints exact instructions if it can't.
...
Run from the directory containing Dockerfile + full_pipeline.py + entrypoint.sh
(i.e. assets/airflow/).
"""
...
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))           # assets/airflow/
_BUILD_CONTEXT = _SCRIPT_DIR                                       # Dockerfile sits next to the DAG.
...
def discover_config():
            ...
            if len(active_tokens) >= 2:
                ...
                resp = identity.create_auth_token(
                    oci.identity.models.CreateAuthTokenDetails(
                        description="rappi-aidp-airflow deploy"
                    ),
                    user_ocid,
                )
    ...
    # AIDP runtime — required for the DAG to know which jobs to trigger.
    def _require(env_key, label):
    ...
    # Airflow webserver + scheduler need headroom; 2 OCPU / 4 GB is a comfortable demo floor.
    cpus = os.environ.get("DEPLOY_CPUS", "2")
    memory_gb = os.environ.get("DEPLOY_MEMORY_GB", "4")
    display_name = os.environ.get("DEPLOY_DISPLAY_NAME", "rappi-aidp-airflow")
...
def build_and_push(cfg):
    ...
    # Build context is assets/airflow/ — Dockerfile, entrypoint.sh, and
    # full_pipeline.py all live there and the COPY directives are relative to it.
        ...
        oci.container_instances.models.CreateContainerInstanceDetails(
            ...
            containers=[
                oci.container_instances.models.CreateContainerDetails(
                    ...
                    environment_variables=_build_container_env(cfg),
                )
            ],
            vnics=[
...
def main():
    print("=" * 60)
    print("  Rappi AIDP Airflow  |  Deploy to OCI Container Instances")
    print("=" * 60)

    cfg = discover_config()

    # IAM check skipped — reusing the Dynamic Group + Policy from the
    # aidp-agent-chat-ui sample (same compartment, same DG matching rule,
    # same 'use ai-data-platforms' grant). Re-enable if deploying into a
    # compartment that doesn't already have those resources.
...
""")
  ...
  │                                                                     │
  │  The IAM Dynamic Group + 'use ai-data-platforms' policy gate the    │
  │  AIDP control plane. If a triggered jobRun still fails with         │
  │  forbidden, add the DG below as a member of the AIDP workspace      │
  │  (AIDP Console → Workspace → Roles → add → paste the DG OCID).      │
  │                                                                     │
  │  DG OCID:                                                           │
  │    {dg}
  │                                                                     │
  ...
  Airflow login (set via AIRFLOW_ADMIN_USER / AIRFLOW_ADMIN_PASSWORD):
    user: {admin_user}
    pass: {admin_pass}
""")
  ...
  Access at:  http://{vnic.public_ip}:8080

  Note: this is HTTP, not HTTPS, and Airflow's built-in auth is the only
  thing in front of it. For production, place a load balancer with HTTPS
  in front, or restrict the subnet's security list to known source IPs.
""")
                _print_post_deploy_notes()
                return
    except Exception:
        pass
